...
export default async function CardBuilderPage({ searchParams }: { searchParams: Promise<{ tour?: string; preview?: string }> }) {
  ...
  return <CardBuilderV2 initialCard={card} demo={viewer.demo} siteUrl={brand.siteUrl} planId={access.plan} accountEmail={viewer.email} onboardingSeenAt={viewer.onboardingSeenAt} locked={access.locked} lockReason={access.reason} trialActive={access.trial.active} forceTour={query.tour === "1"} initialPreviewMode={query.preview === "desktop" ? "desktop" : "mobile"} />;
}
