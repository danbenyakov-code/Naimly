  ...
  return (
    <div className="mx-auto max-w-[1060px]">
      ...
      <div className="mt-6 grid gap-5">
        ...
        <section className="card-surface p-5 sm:p-7"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#e9fbf7] text-[#08735f]"><CreditCard size={21} /></span><div><h2 className="font-extrabold">המסלול שלך: {plan.name}</h2><p className="text-sm text-[#6f7c90]">{plan.price ? `${formatCurrency(plan.price)} לחודש` : "תקופת ניסיון"} · {viewer.subscriptionStatus === "active" ? "פעיל" : trial.expired ? `ההתנסות בת ${TRIAL_DAYS} הימים הסתיימה` : trial.active ? `בתקופת התנסות — נותרו ${trial.daysLeft} ימים` : viewer.subscriptionStatus}</p></div></div><Link href="/pricing" className="button-secondary">{trial.active || trial.expired ? "בחירת מסלול" : "שינוי מסלול"}</Link></div>{(trial.active || trial.expired) && <div className="mt-5"><TrialTimer endsAt={trial.endsAt} /></div>}<div className="mt-5 grid gap-2 sm:grid-cols-2">{plan.features.slice(0, 6).map((feature) => <span key={feature} className="flex items-center gap-2 text-sm text-[#5f6d83]"><CheckCircle2 size={16} className="text-[#0a9b81]" />{feature}</span>)}</div></section>
