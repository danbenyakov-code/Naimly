...
import { Accessibility, ALargeSmall, Contrast, Link2, Mail, MessageCircle, MessageSquareText, Minus, Plus, RotateCcw, X } from "lucide-react";
...
export function FloatingTools() {
  ...
  const whatsappHref = brand.supportWhatsapp ? `https://wa.me/${brand.supportWhatsapp.replace(/\D/g, "")}` : "";
  return <>
    ...
    <div className="floating-contact" role="group" aria-label="יצירת קשר">
      {whatsappHref && (
        <a href={whatsappHref} target="_blank" rel="noopener noreferrer" className="floating-contact-item floating-contact-whatsapp" aria-label="שליחת הודעה בוואטסאפ">
          <MessageCircle aria-hidden="true" size={19} />
        </a>
      )}
      <a href={`mailto:${brand.supportEmail}`} className="floating-contact-item" aria-label={`שליחת מייל אל ${brand.supportEmail}`}>
        <Mail aria-hidden="true" size={19} />
      </a>
      <Link href="/contact" className="floating-contact-item floating-contact-form" aria-label="מעבר לטופס יצירת קשר">
