...
`smtp_pass` הוא היוצא מן הכלל: ה-API מחזיר עליו **digest** ולא את הערך
שנכתב, ולכן השוואה עליו חסרת משמעות. הוא מסומן write-only ונכתב בכל הרצה,
והאימות שלו הוא פונקציונלי:

```bash
npm run supabase:auth -- --test-email you@example.com
```

הפקודה מבקשת מ-Supabase לשלוח מייל שחזור אמיתי. אם פרטי ה-SMTP שגויים
Supabase מחזירה `HTTP 500`; קבלה נקייה פירושה שהאימות מול Gmail עבר.
